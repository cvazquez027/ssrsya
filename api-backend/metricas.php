...
try {
    ...
    if (!$sigla) throw new Exception("Sigla es requerida");
    ...
    // --- CIRUGÍA: Filtro RLS combinado con la sigla solicitada ---
    $filtro = getSQLFilter($pdo, $id_usuario, $rol_planificacion, 'p.sigla_dependencia');
    $whereFiltro = is_array($filtro) ? $filtro['sql'] : "";
    $paramsBase = is_array($filtro) ? $filtro['params'] : [];

    // Función para juntar la sigla buscada con los parámetros RLS
    $getParams = function() use ($sigla, $paramsBase) {
        return array_merge([$sigla], $paramsBase);
    };

    // 1. Proyectos
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM proyecto p WHERE p.sigla_dependencia = ? " . $whereFiltro);
    $stmt->execute($getParams());
    $proyectos = $stmt->fetch()['count'];

    // 2. OEs
    $sqlOE = "SELECT COUNT(DISTINCT oe.id_oe) as count 
              ...
              WHERE p.sigla_dependencia = ? " . $whereFiltro;
    $stmt = $pdo->prepare($sqlOE);
    $stmt->execute($getParams());
    $obsEspecificos = $stmt->fetch()['count'];

    // 3. Actividades
    $sqlAct = "SELECT COUNT(DISTINCT a.id_actividad) as count 
    ...
    // 4. Indicadores
    $sqlInd = "SELECT COUNT(DISTINCT i.id_indicador) as count 
               FROM indicador i 
    ...
    $sqlAvance = "SELECT 
                  ...
                  WHERE p.estado_proyecto = 3 AND p.sigla_dependencia = ? " . $whereFiltro;

    $stmtAvance = $pdo->prepare($sqlAvance);
    $stmtAvance->execute($getParams());
    ...
    foreach ($monitoreos as $row) {
        ...
        if (!isset($proyectos_data[$p_id])) {
            $proyectos_data[$p_id] = ['nombre' => $row['proyecto_nombre'], 'indicadores' => []];
        }
        ...
        if ($row['meta_propuesta'] !== null || $row['meta_alcanzada'] !== null || $row['no_aplica'] !== null) {
            $proyectos_data[$p_id]['indicadores'][$i_id][] = [
                'prop' => $row['meta_propuesta'], 'alc' => $row['meta_alcanzada'], 'na' => $row['no_aplica']
            ];
        }
    }
    ...
    foreach ($proyectos_data as $p_id => $p_data) {
        ...
        $proyectos_avance[] = ['nombre' => $p_data['nombre'], 'avance' => ($count_p > 0) ? round($sum_p / $count_p, 1) : 0];
    }

    usort($proyectos_avance, function($a, $b) { return $b['avance'] <=> $a['avance']; });
