...
export function FormularioEstado({ onClose, editingItem }: FormularioEstadoProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editingItem
        ? JSON.stringify({ id_estado: editingItem.id_estado, descripcion })
      ...
      toast({
        title: "Éxito",
        description: `Estado ${editingItem ? "actualizado" : "creado"} correctamente`,
      })
      onClose()
    } catch (error) {
      console.error("[v0] Error saving estado:", error)
      toast({
        title: "Error",
    ...
    <Card className="bg-card">
      <div className="p-6">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-2xl font-semibold text-foreground">{editingItem ? "Editar" : "Nuevo"} Estado</h2>
