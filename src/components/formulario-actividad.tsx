...
export function FormularioActividad({ onClose, editingItem, defaultOE }: FormularioActividadProps) {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/objetivo-especifico").then((res) => res.json()),
      fetch("/api/tipo-actividad-prioritaria").then((res) => res.json()),
    ])
      .then(([oeData, tipoData]) => {
        setObjetivosEspecificos(oeData)
        setTiposActividad(tipoData)
        setDataLoaded(true)
